...
<form name="oauth2_authorize_form" id="oauth2_authorize_form" action="<?php echo esc_url( $url ); ?>" method="get">
  ...
  <h2 class="login-title"><?php echo esc_html( sprintf( __( 'Authorize %1$s', 'oauth2' ), $consumer->post_title ) ) ?></h2>
  ...
  <div class="login-info">
	...
	<p><?php
	  printf(
	  __( 'Howdy <strong>%1$s</strong>,<br/> <em>"%2$s"</em> would like to connect to <em>"%3$s"</em>.' ), $current_user->user_login, $consumer->post_title, get_bloginfo( 'name' )
	  )
	  ?></p>
  </div>
